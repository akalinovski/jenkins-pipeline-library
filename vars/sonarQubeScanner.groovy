    ...
    if (runSonarScanner) {
        try {
            ...
            dir(tmpDir) {
                ...
                sh "java -jar ${localScanner}  -Dsonar.host.url=http://${serviceName}:${port}  -Dsonar.projectKey=${jobName} -Dsonar.projectBaseDir=${srcDirectory} -Dsonar.java.binaries=${srcDirectory}/target/classes -Dsonar.sources=${srcDirectory}/src -Dsonar.login=admin -Dsonar.password=admin"
            }
